/**
  Instant Site Widget Builder Integration
  ...
  standalone: bigger bundle (~1.7MB, this app pulls in MUI/Emotion), self-contained.

  apiUrl is this repo's own worker/src/routes/instant-site.ts. embedToken must
  match the INSTANT_SITE_EMBED_TOKEN secret set on that worker.
*/
...
// (do NOT include a wrapping function(...) { } yourself):
  ...
  var props = {
    ...
    embedToken: data.config.embedToken,
  };
  ...
  fetch('https://YOUR_WORKER_DOMAIN/react-mode?app=instant-site')
    ...
    .then(function (settings) {
      ...
      var scriptSrc = reactMode === 'standalone'
        ? 'https://YOUR_WORKER_DOMAIN/bundles/instant-site/dm-widget.standalone.js'
        : 'https://YOUR_WORKER_DOMAIN/bundles/instant-site/dm-widget.js';
...
// ──────────────|───────────|──────────────────────────────────────────────────────────
// apiUrl        │ Text      │ https://YOUR_BACKEND_DOMAIN
// embedToken    │ Text      │ Must match the INSTANT_SITE_EMBED_TOKEN secret set on the
//               │           │ ext-app-widget-backend worker (see worker/README.md)
// enablePresets │ Checkbox  │ DEV ONLY. Enables the "Fill with example…" autofill
//               │           │ dropdown. Leave unchecked on live sites.
//
